...
class PlannerPrompt:
    """Constructs the prompt for the strategic reflection (planner) step, preserving the rich context."""
    
    TEMPLATE = """Role: Adaptable Strategist & Reflector for AI Web/File Automation Agent.

Internal Analysis (Pre-JSON):
- Situation Recap: Distill developments, progress, findings, obstacles from recent_history, previous_plan, and task.
- Goal Alignment Check: Verify actions vs. task. Flag deviations. Prioritize error_context if provided.
- Bottleneck & Opportunity Scan: Pinpoint constraints, failure points, or unexpected opportunities.

Operational Directive:
- Your output MUST be a single, valid JSON object. No other text or explanation is allowed.
...
error_context_for_planner: {error_context_for_planner}

**Required Output Format (JSON object ONLY):**
```json
{{
  "memory_summary": "A concise but comprehensive summary of key progress, findings, and obstacles. This will be the agent's memory for the next set of steps.",
  "next_goal": "A clear, actionable, high-level strategic goal for the next phase of the task. This will guide the agent's next actions.",
  "effective_strategy": "Optional: A brief, reusable strategy or learning that emerged. Null if none."
}}
```"""

    def __init__(self, task: str, history: 'AgentHistoryList', last_error: Optional[str]):
        self.task = task
        self.history = history
        self.last_error = last_error

    def get_messages(self) -> List[BaseMessage]:
        """Formats the inputs into a list of messages for the LLM, preserving the detailed input spec."""
        ...
        # 2. Extract the current URL from the last known state
        current_url = "Not available"
        if self.history.history and self.history.history[-1].state:
            current_url = self.history.history[-1].state.url or "about:blank"

        # 3. Format the final prompt using all available information
        prompt_text = self.TEMPLATE.format(
            task=self.task,
            previous_plan="Not available in this context. You must create a new plan based on the history.",
            recent_history=recent_history_str,
            current_url=current_url,
            error_context_for_planner=self.last_error or "None",
        )
